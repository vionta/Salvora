...
Salvora is a tool to manage document collections, content and XForms directly from local folders, without embedding them in a full application server solution.

You can use Salvora to gather data and modify documents using XForms both locally or for small groups \(like in a VPN protected network\). You can also use Salvora for prototyping document based applications or in order to prepare Xslt and XProc staging solutions. Salvora is not intended primarily to deploy wide available production environments and the external access switch is disabled by default.

Current browsers tend to limit some of the functionalities of XForms \(XSLTForms http://www.agencexml.com/xsltforms.htm for more details about XSLTForms\) when run directly from local file systems. Current Html standard and browsers only support Xslt 1.0.
...
-   Run simple XForms in controled environments with simple configurations.
...
-   java -jar salvora-fat-%lt;version\>.jar -p:8085 -e:true
  ...
  <collection
	...
	name="issues-index" base-path="index"  write-allowed="false" >
	  ...
	  <trigger name="file-indek" before="true"
		   source="file:/home/ws/salvora-playground/issue-list/trigger/issue-list.xpl" />
    </collection>
...
Salvora is a tool to manage document collections, content, XForms, Xslt Transformations and XProc process directly from local folders, without embedding them in a full application server solution.
...
Current browsers tend to limit some of the functionalities of XForms \(XSLTForms http://www.agencexml.com/xsltforms.htm for more details about XSLTForms\) when run directly from local file systems and the current Html standard and browsers only support Xslt 1.0.
...
-   Salvora primary purpose is to run simple XForms in controled environments with simple configurations.
-   Salvora can be used to build small applications, to capture or share Xforms based data like team work forms, ddocument collections data from some department or group, polls, etc.
-   Salvora is useful with management data using semantic techniques as a step up from cell based formulas in Excell spreadsheets .
-   Salvora can also be used for testing and developing Forms and browser XSLTs without deployments. Also some processing capability is added using XProc iii triggers on file and transformation calls.
...
-   Salvora primary purpose is to run simple XForms with simple configurations.
-   Salvora can be used to build small applications, to capture or share Xforms based data like team work forms, ddocument collections data from some department or group, polls, etc.
-   Salvora is useful with management data using semantic techniques as a step up from cell based formulas in Excell spreadsheets .
-   Salvora can also be used for testing and developing Forms and browser XSLTs without deployments. Also some processing capability is added using XProc iii triggers on file and transformation calls.
...
-   Salvora is useful for management data using semantic techniques as a step up from in Excell spreadsheets. Cell based calculation tend to be buggy and dificult to debug on the long term, when data volume and changes accumulate over time.
-   Salvora can also be used for testing and developing Forms and browser XSLTs without deployments. Also some processing capability is added using XProc iii triggers on file and transformation calls.
